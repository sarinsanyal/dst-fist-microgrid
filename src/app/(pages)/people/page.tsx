...
function MemberCard({ member }: { member: Member }) {
    return (
        <div className="flex flex-col items-center text-center bg-white border border-ink/10 rounded-xl p-6 shadow-sm hover:shadow-md transition-shadow">
            ...
            <h3 className="font-serif text-lg font-bold text-ink">{member.full_name}</h3>
            <p className="text-sm text-red-primary font-semibold mt-0.5">{member.designation}</p>
...
export default async function PeoplePage() {
    ...
    const { data: members } = await supabase
        .from("profiles")
        .select("id, full_name, role, designation, email, specialties, google_scholar, linkedin, avatar_url, sub_group, group_id, status")
        .eq("status", "approved")
        .neq("role", "admin")
        .order("created_at", { ascending: true });

    const grouped = ROLE_ORDER.reduce<Record<string, Member[]>>((acc, designation) => {
        const group = (members ?? []).filter((m) => m.designation === designation);
        if (group.length > 0) acc[designation] = group;
        return acc;
    }, {});
